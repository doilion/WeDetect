...
# 评测函数
# -------------------------------
def evaluate_retrieval_per_class(predictions: Dict[str, List[int]], 
                                gt: Dict[str, Set[int]]) -> Dict[str, Dict[str, float]]:
    """
    对每个类别计算 Precision, Recall, F1
    """
    results = {}
    all_classes = set(gt.keys())
    
    for cat_name in tqdm(all_classes, desc="Evaluating categories"):
        pred_img_ids: List[int] = predictions.get(cat_name, [])
        pred_set: Set[int] = set(map(int, pred_img_ids))  # 确保是 int
        gt_set: Set[int] = gt[cat_name]
...
if args.dataset == 'coco':
    ...
    ANNOTATION_FILE = 'data/coco/annotations/instances_val2017.json'  # COCO val2017 注解文件
    ...
    # 构建每个类别的 GT 图像 ID 集合
    gt_by_category: Dict[str, Set[int]] = defaultdict(set)

    for cat_name in cat_name_to_id:
        cat_id = cat_name_to_id[cat_name]
        img_ids = set(coco.getImgIds(catIds=cat_id))
        if len(img_ids) > 0:
            gt_by_category[cat_name] = img_ids

    print(f"Loaded GT for {len(gt_by_category)} categories.")
    ...
    ANNOTATION_FILE = 'data/coco/lvis/lvis_v1_val.json'  # 请确保路径正确
    ...
    cats = lvis.load_cats(cat_ids)
    cat_name_to_id = {cat['name']: cat['id'] for cat in cats}
    cat_id_to_name = {cat['id']: cat['name'] for cat in cats}

    print(f"Loaded {len(cats)} categories from LVIS.")

    # -------------------------------
    # 构建每个类别的 GT 图像 ID 集合
    # 注意：LVIS 是 federated 标注，未标注 ≠ 不存在，但这里采用 closed-world 假设
    # -------------------------------
    gt_by_category: dict[str, set[int]] = defaultdict(set)

    for cat_name in cat_name_to_id:
        cat_id = cat_name_to_id[cat_name]
        # 获取所有属于该类别的 annotation IDs
        ...
        # img_ids = set(lvis.get_img_ids([cat_id]))  # 获取标注了该类的图像 ID
        if len(img_ids) > 0:
            gt_by_category[cat_name] = img_ids

    print(f"Loaded GT for {len(gt_by_category)} categories.")
    classnames = ['aerosol_can', 'air_conditioner', 'airplane', 'alarm_clock',
...
pred = torch.load(f"{args.dataset}_{args.model}.pth", 'cpu')
text_embedding = pred['text_embedding']

PREDICTIONS = {name: [] for name in classnames}

for result in pred['image_embedding']:
    image_id = result['image_id']
    cls_logits = torch.einsum('bw,kw->bk', result['embedding'], text_embedding)
    if args.model == 'hqclip': 
        cls_logits = torch.sigmoid(cls_logits)
    else:
        cls_logits = torch.sigmoid(cls_logits * result['scale'].exp().unsqueeze(1) + result['bias'].unsqueeze(1))
    cls_logits = torch.max(cls_logits, dim=0)[0]
    cls_ids = torch.where(cls_logits > args.thre)[0].tolist()
    for ids in cls_ids:
        PREDICTIONS[classnames[ids]].append(image_id)


print("Starting evaluation...")
results = evaluate_retrieval_per_class(PREDICTIONS, gt_by_category)

print("\n" + "="*80)
print(f"{'Class':<15} {'Prec':<8} {'Recall':<8} {'F1':<8} {'Support':<8} {'Pred#':<8}")
print("-"*80)
for cat_name in sorted(results.keys(), key=lambda x: results[x]['f1'], reverse=True):
    r = results[cat_name]
    print(f"{cat_name:<15} {r['precision']:<8} {r['recall']:<8} {r['f1']:<8} {r['support']:<8} {r['n_pred']:<8}")
...
print("-"*80)
print(f"{'Macro Avg':<15} {macro_p:<8.4f} {macro_r:<8.4f} {macro_f1:<8.4f}")
print("="*80)
